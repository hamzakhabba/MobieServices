...
describe('ReservationsListComponent', () => {
  ...
  it('should call getReservationObs and setReservationsList on initialization', fakeAsync(() => {
    const getReservationObsSpy = jest.spyOn(
      reservationService,
      'getReservationObs'
    );
    component.ngOnInit();
    ...
    const mockClientReservations = [
      {
        ...
        travelDate: new Date('2023-12-28'),
        busId: 1,
      },
    ];
    const getAllClientReservationsSpy = jest
      .spyOn(reservationService, 'getAllClientReservations')
      .mockReturnValue(of(mockClientReservations));
    component.ngOnInit();
    tick();
  ...
  it('should apply modifications to reservation', () => {
    ...
    expect(component.reservations[0].travelDate).toEqual(
      new Date('2023-12-28')
    );
  });
