...
export class ReservationsListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.reservationService.getReservationObs().subscribe((reservation) => {
    ...
    this.subscription.add(
      this.reservationService
        .getAllClientReservations(this.CLIENT_ID)
        .subscribe(
          (reservations) => {
  ...
  modifyReservation(selectedReservation: Reservation): void {
    this.reservations.forEach(
      (item) => (item['edit'] = selectedReservation === item)
    );
  }
  apply(selectedReservation: Reservation): void {
    selectedReservation['busId'] = parseInt(
      this.busModification.value ?? '',
      10
    );
    selectedReservation['travelDate'] = new Date(
